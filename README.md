...
- 迭代 13（文本排版与二维码参数持久化）：元素契约补齐（wrap / lineHeight / fitMode / fontFamily / qrEcc / qrMargin / displayValue / paddingH-V）+ Skia 图片打印渲染 + 前端字段映射，前后端已完成（用户验收待执行）；产物 `LabelFrame-0.12.2.msi`。
...
产物：`artifacts\LabelFrame-0.12.2.msi`（约 10MB，x64 framework-dependent，含 WinHost + Web UI + 应用图标；安装到 `C:\Program Files\LabelFrame`）。`appsettings.json` 为独立用户配置组件：覆盖安装 / 修复不覆盖、卸载保留，升级不会丢用户配置。
