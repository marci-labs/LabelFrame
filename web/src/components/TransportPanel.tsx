// 连接方式（迭代 15 §6.2）：设置页完整面板 + DataPrint 顶部快速切换。
...
export function TransportPanel() {
  ...
  const run = async (testOnly: boolean) => {
    ...
    setResult(null)
    try {
      const r = testOnly ? await api.testTransport(form.buildRequest()) : await api.setTransport(form.buildRequest())
      if (r.ok) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        {!app.transportConfig && <span className="hint">（旧版后端无连接管理端点，仅显示健康检查模式）</span>}
      </div>
      ...
      <div className="hint">切换为先测试后生效：测试失败不切换并提示原因；保存后重启 WinHost 仍保留该连接。</div>
    </div>
  )
}
...
export function TransportQuickSwitch() {
  ...
  const apply = async () => {
    ...
    setErrMsg(null)
    try {
      const r = await api.setTransport(form.buildRequest())
      if (r.ok) {
        app.applyTransportConfig(r.config)
