...
- **真机走查（UROVO DT50，Release APK）**：主页（绿色状态卡「一切正常，随时可以打印」+ 三入口行摘要）、服务器子页（真实地址回填 + 引导句）、测试连接（绿字「✓ 能连上服务器」）、打印机子页（IP / 端口 / 引导句）、**保存并重启服务（现场实际操作：新地址持久化 + 服务带新配置运行）**、常驻通知新文案（dumpsys 核对）、浏览器状态页（标题 / 副标题 / 按钮 / 加载文案）逐项核验通过；测试打印完整链路 API 确认 **Completed（1/1，真实 Zebra 出纸）**。屏上打印结果文案 / 本机信息子页 / 未保存提示未逐屏截屏（走查时设备在使用中），留待日常使用确认。
