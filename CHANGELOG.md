...
- **会话内缓存与生命周期**：「模板名 → blob URL」会话内缓存，同一列表周期内重复悬停不重复请求（含在途去重与失败结果缓存）；列表刷新（重新 load，如删除 / 导入后）缓存整体失效并释放全部 blob URL（防泄漏），周期代数防护使失效时仍在途的旧请求结果不写入新周期；组件卸载兜底释放。
- **加载 / 失败态**：拉取中浮层显示「正在生成预览…」；失败（模板已删除、旧宿主 404、网络不可达）显示「预览不可用」+ 错误消息占位提示，不影响列表与其余功能；预览图透明底衬浅棋盘格（白底标签不「隐形」）。
- **测试**：新增前端测试 6 项——防抖边界（399ms 不发 / 400ms 发）与加载→图片态、单机降级走本机 API、缓存命中不重复请求不重复建 blob URL、列表刷新失效（释放旧 URL 并重拉）、失败态与同周期失败缓存、快速扫行无请求风暴；日常 `dotnet test` 364 项全绿（排除 Perf / Soak），前端 `pnpm lint / test`（client / server 双模式各 256 项）与双构建通过。
