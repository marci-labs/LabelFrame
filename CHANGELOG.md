...
- **测试**：新增单元（FakeTimeProvider）+ HTTP 集成测试 11 项——失联回收终态 / 正常回报零回归 / 只回收失联 Claimed / 迟到回报幂等重放 / 关闭开关回归 / 配置项默认值与环境变量覆盖 / 端到端（领取 → 失联 → 回收 → 查询终态与原因 → 无二次领取）；日常 `dotnet test` 338 项全绿（排除 Perf / Soak）。
