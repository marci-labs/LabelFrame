...
- **预览列内嵌缩略图（验收修订：纯悬停触发不可发现，用户定稿 B 形态）**：工作台新增「预览」列，列表加载后按需调用既有 `POST /api/templates/{name}/preview`（按模板 TestData 渲染 PNG，Server 与 WinHost 双宿主均已实现）拉取全部模板预览，单元格内以 40px 高等比缩略图直出（所见即所得）；**点击缩略图放大**——行旁浮层展示大图（视口不足自动翻到左侧），半透明遮罩 / Esc / 点击浮层均可关闭。初版为行悬停 400ms 防抖触发的浮层（PR #19），真机验收时用户判定不可发现，当轮修订为预览列形态（验收教训：客户端托管 UI 的验收须先重建 web/dist）。前端新增 `previewTemplate(name)` blob 封装（与 renderImage 同构，serverApi / localApi 双 base 自动跟随业务模式——Client 与 Server UI 共享工作台组件，双端同时受益）。
