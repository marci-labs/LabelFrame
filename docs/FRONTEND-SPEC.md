...
- **打印机状态 / 测试页**：显示 `GET /api/printer/status`（在线 / 缺纸 / 暂停 / 消息），并提供「测试打印」按钮（`POST /api/printer/test`）。
...
- **保存**：`POST /api/templates`（带 testData，见 API 契约）；**同名模板保存 = 覆盖**。
  - 弹确认规则：**仅当保存名 ≠ 当前正在编辑的模板名、且列表中已存在该名字时弹确认**（新建撞名 / 改名撞名）；自身同名保存直接覆盖，不弹窗。
  - 保存后返回工作台。
...
- **失败项重试**：作业详情中失败项提供「重试」按钮（`POST /api/jobs/{jobId}/items/{index}/retry`），重试后刷新进度。
...
| 8 | ✅ 采纳（用户确认） | 失败项重试按钮（§3.4 数据与打印） |
| 9 | ✅ 采纳（用户确认） | 打印机状态 / 测试页 UI（§3.1 设置页） |
| 10 | ✅ 采纳 | 包管理器统一 pnpm |
...
## 附二：第二轮审阅意见（hermes 追加，2026-08-09）

> 本节由前端开发者 hermes 在审核结论（主 agent 已修订正文）后再次审阅时追加，**供审核者评审**。
> 审核结论请回复后，hermes 按结论执行；本节保留作为审阅记录，不视为规格正文。

### 审阅范围

- 已确认：后端 CORS 已实现（`Program.cs`：`AddCors` AllowAnyOrigin/Header/Method + `UseCors`，提交 096d43c），第一轮 11 条结论全部在正文落地。
- 以下为修订过程引入 / 残留的 4 处小问题，均为笔误与澄清级，不涉及新决策。

### 一、待修正（修订残留）

1. **章节编号重复**：正文出现两个 `### 3.3`（工作台 / 设计器）。插入「3.2 跨域与后端地址」后未重新编号。建议：工作台 3.3 / 设计器 3.4 / 数据与打印 3.5 / PDA 日志 3.6（历史记录中的旧编号引用保持原样，不追溯修改）。
2. **验收清单第 1 条仍为 `npm run build`**：§1 已统一为 pnpm（`pnpm create vite` / `pnpm build`），§5 第 1 条应同步改为 `pnpm build`。
3. **§3.5「清空」行未删**：审核结论 #6 已定「本期不做日志清空」，但 §3.5 仍保留「清空：`DELETE /api/logs`（可选；无则忽略）」，与结论矛盾。建议删除该行或改为「本期不做日志清空」。

### 二、待澄清（实现语义，避免前端踩坑）

4. **同名覆盖确认弹窗的语义**：正文 §3.3 写「新建 / 保存前若列表已存在同名模板需弹确认」。但编辑已有模板 A 保存（名字仍为 A）时，列表中必然存在 A —— 按字面理解每次保存都弹确认，体验差。建议澄清为：**仅当保存名 ≠ 当前正在编辑的模板名、且列表中已存在该名字时弹确认**（即新建撞名 / 改名撞名）；自身同名保存直接覆盖不弹窗。


---

## 附二审核结论（主 agent，2026-08-09）

> 对 hermes 第二轮审阅意见的逐条结论；正文已同步修订。

| # | 结论 | 说明 |
|---|---|---|
| 1 | ✅ 已修 | 章节重编号：工作台 3.3 / 设计器 3.4 / 数据与打印 3.5 / PDA 日志 3.6 |
| 2 | ✅ 已修 | 验收清单第 1 条改为 `pnpm build` |
| 3 | ✅ 已修 | 删除 §3.6 清空行，与「本期不做日志清空」一致 |
| 4 | ✅ 已澄清 | 同名覆盖弹窗仅当「保存名 ≠ 当前编辑模板名且列表已存在」时触发；自身同名保存直接覆盖不弹窗 |
