...
- 传输插件分发（2026-08-17 已排入迭代 23，范围讨论中）：把迭代 22 的插件机制做成可分发闭环——插件包上传服务端（复用 / 扩展 client-packages 或独立插件目录 + API）、客户端界面选择安装（下载 → 放入插件目录 → 重启生效）与卸载（删除 → 重启生效）；插件包格式 / 版本 / 校验（哈希或签名）会话中定稿；与「更新与安装包」（迭代 22）UI 归并或并列待定。
