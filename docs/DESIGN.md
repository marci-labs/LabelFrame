...
- 工程治理遗留项（2026-08-25 评审 + 迭代 27 P0 完成后排期）：P1——docs/ 历史迭代 SPEC 归档分层与 ROADMAP 章节排序 / 陈旧状态修复、Studio（WPF）与 prototypes/web-designer 移除决策、ClientPackagesService / PluginPackagesService 同构去重、真机验收清欠（迭代 2/4/13/14/15/16 长期挂「验收待执行」）；P2——Program.cs 巨文件拆分 + WebApplicationFactory 端点集成测试、全局异常处理与 ProblemDetails、数据层 N+1 / 领取事务 / ServerService 全局信号量、TransportManager 状态加锁、局域网信任模型显式化（插件 / 安装包分发无鉴权无签名的威胁模型记录）。
