...
- **AOT 动态加载前提（Spike 假设，AC-02）**：Release AOT 构建下动态加载程序集走 interpreter 解释执行，打印链路性能可接受为**假设**——Spike 真机实证（fake 传输插件完整链路走查）转待验收；若被推翻按 fail-fast 中止机制段转用户评估（轻量档通道是否降级「一个品牌一个 APK」构建变体届时另议）。
