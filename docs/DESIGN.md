...
| 配置 | `GET/POST /api/host/config`（serverUrl / printerBrand / connectionType / tcpHost / tcpPort / deviceName；deviceId 只读自动生成；持久化 SharedPreferences，重启宿主生效） |
| 测试 | `POST /api/host/test-print`（内置测试标签走完整链路：校验 → 渲染 → `^GF` → TCP 发送，返回作业供轮询终态） |
| 其他 | `GET /healthz`、`GET /`（内置宿主状态页：配置概览 + 打印机探测 + 测试打印） |

宿主配置 UI（决策 #95）：原生配置 Activity 为唯一界面（服务端地址 + 打印机「品牌 → 连接类型 → TCP 参数」+ 设备信息只读区），「保存并应用」自动重启宿主服务生效；运行状态、通知文案与状态页同源（进程内 `HostStatus` 快照）。
