...
- 工程治理剩余项（2026-08-25 迭代 27/28 完成 P0-P2 后更新）：C# 侧质量关卡（.editorconfig / analyzers / 覆盖率度量，前端已具备对标）；宿主专属端点（WinHost 连接 / 插件 / 机器级配置，Server 设备 / 作业 / 分发）的 HTTP 集成测试（共享端点层已由 LabelFrame.Api.Tests 覆盖）；ServerService 提交幂等下沉 DB（INSERT OR IGNORE + 唯一索引，去掉进程内门）；插件包签名（触发条件见决策 #79）。真机验收欠账集中管理：docs/ACCEPTANCE-BACKLOG.md。
