...
| 43 | MSI 安装包方案（迭代 10，2026-08-09） | WiX v7 打包：WinHost 发布产物（win-x64 self-contained，含 web/dist）+ 桌面 / 开始菜单快捷方式；WinHost 改为 WinExe（无控制台）+ 启动自动打开浏览器 + 日志写文件 + 本机关闭端点；appsettings 由安装生成（端口 / 传输模式默认 Log / 打印机）；卸载清理数据目录选项 | 干净电脑：安装 MSI → 双击图标 → 浏览器打开即用；依赖（.NET Runtime / WebView2）由 self-contained 与浏览器自带覆盖 |
