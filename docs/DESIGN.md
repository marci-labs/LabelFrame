...
| 43 | MSI 安装包方案（迭代 10，2026-08-09） | WiX v7 打包：WinHost 发布产物（win-x64 self-contained，含 web/dist）+ 桌面 / 开始菜单快捷方式（Target=#WinHostExe 文件引用）；WinHost 为 WinExe（无控制台）+ 启动自动打开浏览器 + host.log + 本机优雅关闭端点 + 启动失败弹窗；名称统一 LabelFrame；应用图标（双色 L 型）嵌入 exe；自签名证书脚本（openssl + .NET 重封装）与可选 MSI 签名；清理脚本 | 干净电脑：安装 MSI → 双击图标 → 浏览器打开即用；正式签名需商业证书 |
