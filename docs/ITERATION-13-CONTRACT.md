...
> 状态：已完成并归档（2026-08-10，Hermes 评估通过，前后端已实施，用户验收待执行）
> 协作：本文件定义**前后端元素 JSON 契约字段**（前端 convert.ts ↔ 后端 C# 模型/转换器）与 **Skia 图片打印渲染语义**；Hermes 评估无异议后，后端已实施 C# + Skia（commit a38ba74），前端已实施 convert.ts 联动（commit 8294bef）。
