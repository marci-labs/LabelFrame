...
| 79 | 插件命令打印实现 3/4——Zebra 编译器·条码与二维码（Code 128 `^BC` / `^BY` 编译：模块宽与高度 DPI 换算 + `displayValue` interpretation 映射 + 中文 `LF_ENC_001` 既有语义零回归；QR `^BQ` 编译：ECC 双处映射 + `qrMargin` 静区映射 + 放大倍数按尺寸与 DPI 换算（版本容量估算）+ UTF-8 中文经 `^CI28`；混合模板整页自包含） | ✅ 已完成（2026-09-17 本地 build / test 全绿（850 项，排除 Perf/Soak），编译器新增 16 用例（理论展开 22 例）+ 宿主集成 2 例；真机扫码对比（AC-04）转待验收（恢复条件 = Zebra 真机 + 扫码枪可得），[#121](https://github.com/marci-labs/LabelFrame/issues/121)） |
| 80 | 服务端地址切换「保存即生效」落地与连接状态一致性（评审 #114 C-2 / B-9 / C-3：`ServerRoutingCoordinator` 路由热切换——保存后重建 poller / worker 注册新服务端并清理旧连接，决策 #141；客户端「三名义」——本机打印服务 / 服务端地址 / 已加入服务端各义一名；数据与打印加入状态随探测周期刷新） | ✅ 已完成（2026-09-17 本地 dotnet build / test 全绿（含热切换集成测试双 stub 服务端模拟 AC-01）+ pnpm lint 0 错误 / 311 项前端测试 / 双构建；运行中切换按实现行为口径以集成测试断言，[#128](https://github.com/marci-labs/LabelFrame/issues/128)） |
| 81 | 「图片预览」名副其实——页内预览弹层与下载入口分离（评审 #114 C-1：数据与打印页点击「图片预览」页内弹层呈现当前字段值渲染图（复用 render-image 实时取图，不建作业不自动下载）+ 弹层内显式「下载」按钮，预览与下载彻底分离，决策 #142；模拟出图批量 zip 链路与工作台缩略图预览现状保留） | ✅ 已完成（2026-09-17 本地 pnpm lint 0 错误 / 318 项前端测试 / 双构建全绿，后端零改动，[#129](https://github.com/marci-labs/LabelFrame/issues/129)） |
| 82 | 打印机连接文案用户化收尾（评审 #114 B-1 / A-1 / B-5 / B-2：Log 传输三处徽标「模拟打印」无「LOG」直出——改插件 Describe 覆盖插件路径、回退路径迭代 73 已就绪，displayText 契约字段不变；插件说明去「联调」+ 前端无参数不追加「（无参数）。」残句；TCP 选项列表「网口打印机（TCP 9100）」，决策 #143） | ✅ 已完成（2026-09-17 本地 dotnet build / test 全绿（856 项，排除 Perf/Soak）+ pnpm lint 0 错误 / 323 项前端测试 / 双构建；AC-01 / AC-02 以前端测试断言覆盖（沙箱浏览器走查为可选项），[#130](https://github.com/marci-labs/LabelFrame/issues/130)） |
