...
> 2026-09-10 迭代 44 验收后用户提出五项候选（详述与讨论见验收会话），排序与组合待下一迭代立项讨论定稿：

- **批次首张不等批间间隔**：开启批次作业时第一张不应等待打印间隔（实测 batchSize=1 / 间隔 5s，首张出纸前也等了 5s；疑似 `BatchPrintPolicy.ShouldPauseBeforeSend` 对已发送数 0 即满足批次倍数判断，小改动）。
- **作业进度增量上报**：进度不再终态一次回报（当前跳变）；属跨端公共契约变更（强化路径，先更新 DESIGN 再改码），用户明示不着急、先讨论方案与时机。
- **设置页布局自适应**：大屏下页面宽度与控件宽度不匹配，空间利用差（前端改动）。
- **日志细化机制**：作业失败时日志信息过粗，需要「必要时更详细」的记录机制（分级 / 失败上下文展开等，设计待讨论）。
- **工作台优化**：模板名搜索、模板缩略图预览（点进前可见大致样式）、操作列右侧无白底的视觉修正；整体信息架构需详细讨论。
