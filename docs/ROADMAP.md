...
**完成记录**（2026-08-09）：原型 v3 完成；二 ~ 四轮修复渲染模型、标尺、平移、控件可见性、吸附定位、二维码、边框内边距通用化、文本模式（缩小适应 / 隐藏）；第五轮：字高独立、内边距上下 / 左右、填充默认固定值（字段 = 键名 + 预览值）、Ctrl+C/V 复制粘贴；headless 自测通过；待用户本机验收。
