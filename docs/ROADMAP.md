...
| 22 | 打印测试体验 + 传输插件化 + 客户端下载分发 | 🔄 本轮进行中（2026-08-17 起，范围讨论中） |
...
## 迭代 22：打印测试体验 + 传输插件化 + 客户端下载分发（本轮，范围讨论中）

**目标**：围绕「易用性 / 权限边界 / 插件化 / 分发」四项收口——打印测试更好上手、客户端与服务的边界明确、传输方式插件化、客户端安装包由服务端分发。

**待讨论范围**（2026-08-17 用户提出，会话中定稿）：
- 打印测试：客户端与服务端「数据与打印」界面新增「下载 Excel 模板」入口，便于直接套用 Excel 导入做打印测试。
- 权限边界：客户端只能选择本机做打印测试（不能选其他客户端）；服务端可自由选设备打印测试。
- 客户端显示本机设备名称。
- 作业历史可见性：客户端只看自己的作业历史，服务端看全部。
- 传输插件化：本轮完成插件机制（如何加载 / 卸载 / 使用、统一接口、参数模型、注册表按需装配）；下一轮实现具体厂商插件（如精成打印机）并测试。
- 客户端下载分发：服务端提供客户端下载界面；更新安装包默认从服务端获取；服务端提供上传安装包目录。

**不在范围**：具体厂商打印机插件实现（迭代 23）；PDA（AndroidHost，延后至迭代 24）。
