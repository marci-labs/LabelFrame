...
| 46 | 工作台模板缩略图预览（悬停按需出图 + 会话内缓存，零契约变更） | ✅ 已完成（2026-09-10 待真机验收 AC-07，[#15](https://github.com/marci-labs/LabelFrame/issues/15)） |
...
> 2026-09-10 迭代 44 验收后用户提出五项候选；其中三项（批次首张节奏 / 设置页布局 / 工作台搜索与操作列）已在迭代 45 交付（[#11](https://github.com/marci-labs/LabelFrame/issues/11)），剩余如下：

- **作业进度增量上报**：进度不再终态一次回报（当前跳变）；属跨端公共契约变更（强化路径，先更新 DESIGN 再改码），用户明示不着急、先讨论方案与时机；建议与日志细化合并为「作业可观测性」迭代。
- **日志细化机制**：作业失败时日志信息过粗，需要「必要时更详细」的记录机制（分级 / 失败上下文展开等，设计待讨论）。
- **工作台缩略图预览与信息架构**：模板缩略图（悬停按需预览，复用 `POST /api/templates/{name}/preview` + 会话内缓存，零契约改动，路线已定）；工作台整体信息架构重构（卡片网格 / 列表+详情面板等）待专门讨论。
