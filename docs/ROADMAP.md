...
| 86 | 设置页「服务端已连接」徽标空地址谎报修复（#128 走查观察项：serverUrl 为空时 serverApi base 落同源打到本机 WinHost 谎报已连接、同页安装包 / 插件区 404 同屏矛盾；初始化与机器级配置加载探测竞态双态。a 案 = 空地址不探测直接判「服务端未连接（单机模式可用）」+ 探测按配置就绪串行化与序号后发优先，决策 #146） | ✅ 已完成（2026-09-18 本地 pnpm lint 0 错误 / 354 项前端测试（双模式）/ 双构建全绿，dotnet build / test（856 项，排除 Perf/Soak）零回归，后端零改动；AC-01 / AC-02 沙箱走查留验收环节，[#142](https://github.com/marci-labs/LabelFrame/issues/142)） |
