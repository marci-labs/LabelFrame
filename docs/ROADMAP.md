...
| 73 | 界面文案用户化——开发者向文案修正（模式名 / 路径 / 扩展名 / 内部机制术语用户化或删除，决策 #138）与连接配置低频收纳（设置页「连接方式」默认折叠为摘要一行） | ✅ 已完成（2026-09-17 本地 lint / 304 项前端测试 / 双构建全绿，行为零变化；用户视角界面走查转待验收，[#108](https://github.com/marci-labs/LabelFrame/issues/108)） |
| 74 | 模拟打印作业数据留痕——Log 模式提交记录模板名与传入数据集合（数据集合去重 + 重复张数；单条 ≈2KB 截断并标注完整长度 #139） | ✅ 已完成（2026-09-17 本地 build / test 全绿，新增 14 项用例实证：单张字段键值 / 批量相同去重 / 相同 + 不同混合 / 键序无关去重 / 超限截断 + 完整长度标注 / 边界不截断 / 空数据占位 / 换行转义 / 未命名回退 / 幂等重放不重复留痕 / 非 Log 模式零留痕（AC-04）；真实客户端当日 host 日志核验（AC-01 实机口径）转待验收，[#109](https://github.com/marci-labs/LabelFrame/issues/109)） |
| 75 | 「PDA 日志 / 设备日志」页面下线——回传链路不存在前的界面收敛（双形态导航入口 + 路由移除、组件与测试删除；`/api/logs` 端点与 logs.db 后端保留，决策 #140） | ✅ 已完成（2026-09-17 本地 lint 0 错误 / 300 项前端测试 / 双构建全绿，后端零改动；客户端与 Server 管理界面真实走查转待验收，[#112](https://github.com/marci-labs/LabelFrame/issues/112)） |
