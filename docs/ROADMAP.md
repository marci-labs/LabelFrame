...
| 40 | 场景化易用性优化（范围待讨论定稿） | 📋 下一轮（2026-09-08 用户发起） |
...
## 迭代 40：场景化易用性优化（下一轮，范围待讨论定稿）

**背景**（2026-09-08 用户发起）：迭代 0-39 完成功能骨架（模板设计 / 作业中心 / 设备路由 / 传输插件 / 双端宿主 / 自动发布），迭代 25 真机验收通过（含真实打印机出纸）。下一阶段主题从「能力补齐」转向「降低操作复杂度」——按使用场景提供最短路径方案。

**目标**：对现有功能与流程做易用性优化——针对不同场景（文员批量、操作工 / 仓管员单张、管理员配置、业务系统集成）提供差异化方案，最大程度降低用户操作复杂度。

**讨论输入**（会话中与用户定稿，不预设结论）：
- 各角色现状操作路径盘点：从打开界面 / 扫码到出纸的实际步数，找出最痛的环节。
- 场景化最短路径候选：如 PDA 扫码即打（扫旧标签原样补打）、模板快选 / 收藏、常用「模板 + 数据」组合记忆、默认目标设备记忆等。
- PDA 屏幕配置界面（迭代 25 遗留：现仅 `/api/host/config`，无 UI）。
- 失败与恢复的一线体验：挂起 / 重打 / 离线暂存对操作工是否无感、错误提示是否可行动。
- REQUIREMENTS §8 成功衡量补充「操作步数」类可测指标（如「操作工从扫码到出纸 ≤ N 次动作」），使「降低复杂度」可量化验收。
...
**启动命令**：
> 继续 LabelFrame 迭代 40（场景化易用性优化）。先读 README.md、AGENTS.md、docs/DESIGN.md、docs/REQUIREMENTS.md、docs/ROADMAP.md（迭代 40 小节）与 docs/ACCEPTANCE-BACKLOG.md；本轮先与用户讨论定稿范围与方案——按角色（文员 / 操作工 / 管理员 / 业务系统）盘点现状操作路径与步数，评估候选优化项的收益与代价，排定优先级，定稿后再实施；涉及跨端公共契约变更的先讨论并更新文档再动代码；提交用 Conventional Commits；不推 tag；不修改发布 / CI 工作流；仓库内容不得出现公司 / 业务线品牌字样。
