...
**完成记录**（2026-08-09）：原型 v3 完成；第二轮修复核心渲染模型（stage 尺寸 = 逻辑尺寸 × 比例尺）；第三轮修复控件不可见根因（Konva 9.3 Text 无 clipFunc → Group clip + 未绑定占位）、标尺画进 Konva、中键平移粘滞；第四轮修复吸附 / 定位逻辑坐标、二维码同步渲染、属性刷新、边框内边距通用化、文本模式收敛（缩小适应 / 溢出显示）；headless 自测通过；待用户本机验收。
