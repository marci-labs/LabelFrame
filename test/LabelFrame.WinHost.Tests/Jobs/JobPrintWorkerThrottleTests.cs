...
/// 判定双通道：① 批次节流日志条数（确定性，证明暂停决策次数与位置）；② 发送时间序列大间隔
/// 落在预期的张序（证明决策确实产生了真实延迟）。
/// </summary>
public class JobPrintWorkerThrottleTests
{
    ...
    public async Task Disabled_should_send_without_pauses()
    {
        ...
        Assert.Empty(PauseIndices(result.Offsets));
    }
    ...
    private static async Task<WorkerRunResult> RunWorkerAsync(PrintSettingsDto settings, params int[] jobSizes)
    {
        ...
        try
        {
            ...
            Assert.True(await WaitForCompletionAsync(queue, jobIds, TimeSpan.FromSeconds(30)), "作业未在超时时间内完成。");
            return new WorkerRunResult(
                transport.SendOffsetsMs.ToList(),
                logProvider.Messages.Count(m => m.Contains("批次节流")));
        }
        finally
        {
    ...
    /// <summary>返回发送间隔 ≥ 阈值的（0 基）发送序号——即「暂停」发生的位置。</summary>
    ...
    private sealed record WorkerRunResult(List<double> Offsets, int ThrottleLogCount);
    ...
    /// <summary>内存日志收集器（断言「批次节流」日志条数）。</summary>
