...
> **修订 v7（2026-09-17）**：worktree / 本地分支回收闭环——「三」合并后回收须「remove --force → 验证目录确实消失 → 残骸补删」；「一」扫描每轮做 worktree 残留对账兜底；`.zcode/automations/` 随本次修订入库版本化。起因：C:\work\OpenCode 累积 18 个 `LabelFrame-wt*` 半删残骸（Windows 下 `git worktree remove` 删不动 web/node_modules——pnpm junction / 文件锁——静默半删）与 18 条已合并本地分支泄漏。
> **修订 v8（2026-09-20，当前生效）**：会话临时产物生命周期——worker 临时文件与沙箱证据限定写自己 worktree（不写主仓 `artifacts/`，随 worktree 回收消灭）；「一」扫描新增主仓 `artifacts/` 对账兜底（`cert/` 白名单）。起因：主仓 `artifacts/` 累积 1.6 GB 过程产物（历史构建产物 / 发布暂存 / 已结项验收证据 / 证书中间 PEM）需人工清理；同批配套：验收轮值 v7（结项清证据目录）与 `create-signing-cert.ps1` 导出后自删 PEM。
