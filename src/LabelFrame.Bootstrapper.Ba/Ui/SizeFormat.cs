...
/// <summary>体积展示格式化（确认页；net48 兼容写法）。</summary>
internal static class SizeFormat
{
    ...
    public static string Format(long bytes) => bytes >= 1024L * 1024 * 1024
        ? string.Format(CultureInfo.InvariantCulture, "{0:0.0} GB", bytes / (1024.0 * 1024 * 1024))
        : string.Format(CultureInfo.InvariantCulture, "{0:0.0} MB", bytes / (1024.0 * 1024));
}
